...
const topbarClient = new Appwrite.Client()
  .setEndpoint(APPWRITE_ENDPOINT)
  .setProject(APPWRITE_PROJECT);

const topbarAccount = new Appwrite.Account(topbarClient);
const topbarDatabases = new Appwrite.Databases(topbarClient);
...
if (!window.__TOPBAR_INIT__) {
  ...
  const onLogoutClick = (e) => {
    ...
    doConfirm().then(async (ok) => {
      ...
      await (async function clearAuthState() {
        ...
        try {
          await topbarAccount.deleteSessions();
        } catch (e) {
          console.warn("deleteSessions:", e?.message || e);
        }
  ...
  async function updateBranchCount() {
    try {
      ...
      const subBranchesCount = branches.filter(
        (branch) => !isCompanyAdminBranch(branch, branch.company_id)
      ).length;
      ...
      console.log(
        `Branch count updated: ${subBranchesCount} sub-branches found`
      );
    } catch (err) {
      ...
      if (
        ...
        err.message.includes("No Appwrite project")
      ) {
  ...
  async function fetchBranches() {
    try {
      const user = await topbarAccount.get();
      ...
      const response = await topbarDatabases.listDocuments(DB_ID, BRANCH_COLL, [
  ...
  try {
    ...
    if (isBranchManager) {
      ...
      const branchesIconAnchor = document.querySelector(
        'a[href="branches.html"]'
      );
      if (branchesIconAnchor) {
        const container =
          branchesIconAnchor.closest(".relative") || branchesIconAnchor;
