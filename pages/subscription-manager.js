class SubscriptionManager {
    ...
    checkSubscriptionStatus() {
        ...
        if (this.subscriptionData.planEnd && now > new Date(this.subscriptionData.planEnd)) {
            this.subscriptionData.status = 'expired';
            if (!this.subscriptionData.isBlocked) { // Only set block time on transition
        ...
        if (this.subscriptionData.paymentDueDate && now > new Date(this.subscriptionData.paymentDueDate)) {
            if (!this.subscriptionData.isBlocked) { // Only set block time on transition
                this.subscriptionData.blockedAt = new Date().toISOString();
            }
            this.subscriptionData.isBlocked = true;
    ...
    updateUI() {
        ...
        if (statusElement) {
            statusElement.textContent = this.subscriptionData.status.replace('_', ' ').toUpperCase();
        }
    ...
    updatePlanDisplay() {
        ...
        planCards.forEach(card => {
            ...
            if (planType === this.subscriptionData.plan && this.subscriptionData.status === 'active') {
    ...
    saveSubscriptionData() {
        localStorage.setItem('subscriptionData', JSON.stringify(this.subscriptionData));
    }
